...
- **Well data — strong, confirmed, statewide, updated nightly:** TWDB's Groundwater Database (GWDB) has inventoried nearly 140,000 wells, with location, depth, well type, owner, driller, construction/completion data, aquifer, water-level and water-quality data, downloadable as pipe-delimited files. Better centralized than Colorado's per-permit well situation.
- **GCD boundaries — also real and centralized:** TWDB publishes actual GCD boundary GIS data — usable for the same point-in-polygon boundary technique already proven in Colorado/Idaho/Utah.
- **The genuine gap:** the actual *rules* (production limits, spacing, permitting) are set independently per-district via each GCD's own management plan — there are 98 confirmed GCDs in Texas, 60 single-county and 38 multi-county, and confirmed GCDs are located within only 173 of 254 counties (81 counties have no GCD at all; coverage isn't statewide). A pure county-name lookup would be right most of the time but subtly wrong at the edges — use the real boundary polygons, not county names, same lesson as Idaho's Large POU districts.
